package com.kazyle.hugohelper.server.function.core.balance.view;

/**
 * <p>
 * <b>WuDiZhuanView</b> is
 * </p>
 *
 * @author Kazyle
 * @version 1.0.0
 * @since 2017/6/3
 */
public class WuDiZhuanView {

    private String block_remark;

    private String is_block;

    private String leiji_jifenbao;

    private String zhuanfa_jifenbao;

    private String jifenbao;

    private String day_zhuanfa_jifenbao;

    public String getBlock_remark() {
        return block_remark;
    }

    public void setBlock_remark(String block_remark) {
        this.block_remark = block_remark;
    }

    public String getIs_block() {
        return is_block;
    }

    public void setIs_block(String is_block) {
        this.is_block = is_block;
    }

    public String getLeiji_jifenbao() {
        return leiji_jifenbao;
    }

    public void setLeiji_jifenbao(String leiji_jifenbao) {
        this.leiji_jifenbao = leiji_jifenbao;
    }

    public String getZhuanfa_jifenbao() {
        return zhuanfa_jifenbao;
    }

    public void setZhuanfa_jifenbao(String zhuanfa_jifenbao) {
        this.zhuanfa_jifenbao = zhuanfa_jifenbao;
    }

    public String getJifenbao() {
        return jifenbao;
    }

    public void setJifenbao(String jifenbao) {
        this.jifenbao = jifenbao;
    }

    public String getDay_zhuanfa_jifenbao() {
        return day_zhuanfa_jifenbao;
    }

    public void setDay_zhuanfa_jifenbao(String day_zhuanfa_jifenbao) {
        this.day_zhuanfa_jifenbao = day_zhuanfa_jifenbao;
    }
}
